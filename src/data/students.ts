import type { Student, GroupName } from '../types';
...
export const STUDENTS_DATA: Student[] = [
    ...
    // 15:00-18:00
    ...['יאיר מיכלין', 'רועי שילה', 'ליא שלומי', 'איינשטיין', 'רוני רגב', 'זוהר סיקרי'].map(name => createStudent(name, 'Sinai', '15:00', '18:00')),
    // 13:00-17:00
    ...
    // 15:00-19:30
    ...['שירה רוזנפלד', 'טליה פרנקו', 'נדב שמואל הדג&apos;ס', 'דניאל מרזל', 'תומר שגיא'].map(name => createStudent(name, 'Ziv', '15:00', '19:30')), // Handle single quote in name if any
    // 16:00-19:00
...
function createStudent(name: string, group: GroupName, start: string, end: string): Student {
    return {
        id: name.trim(), // Use name as ID for simplicity
        name: name.trim(),
        group,
